...
| Otras formas | |
|---|---|
| **Descargar** | Coge la [última versión](https://github.com/FarnoodF/Sleepless/releases/latest), descomprímela en `/Applications` y apruébala en **Ajustes del Sistema → Privacidad y seguridad → Abrir igualmente** (está firmada de forma ad-hoc). |
| **Compilar desde el código** | `git clone https://github.com/FarnoodF/Sleepless.git && cd Sleepless && ./install.sh` (sin aviso de Gatekeeper). |
