...
| Weitere Wege | |
|---|---|
| **Herunterladen** | Hol dir das [latest release](https://github.com/FarnoodF/Sleepless/releases/latest), entpacke es nach `/Applications` und bestätige es dann unter **Systemeinstellungen → Datenschutz & Sicherheit → Trotzdem öffnen** (es ist ad-hoc signiert). |
| **Aus dem Quellcode bauen** | `git clone https://github.com/FarnoodF/Sleepless.git && cd Sleepless && ./install.sh` (kein Gatekeeper-Hinweis). |
