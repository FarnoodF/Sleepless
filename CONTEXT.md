...
- **System sleep vs display sleep**: Two independent macOS power domains. `pmset disablesleep` blocks only system sleep, so the Mac keeps running while the display is free to turn off. Sleepless keeps the system awake but lets (and on lid close, makes) the display sleep so a kept-awake Mac draws no display power.
- **Display-off on lid close**: While in the keep-awake state, Sleepless turns displays off the moment the lid closes (via `pmset displaysleepnow`, which needs no privilege), keeping the system active in the background with no screen power draw.
