...
final class PowerController {
    ...
    // Put the display(s) to sleep immediately. This is an ACTION verb (not a setting
    // change), so unlike `pmset -a disablesleep`, it needs no root and no sudoers grant —
    // it only asks powerd to idle the display now. The system stays awake; only the panel
    // goes dark, so a kept-awake Mac with the lid closed draws no display power.
    func sleepDisplayNow() {
        let res = ShellRunner.run("/usr/bin/pmset", ["displaysleepnow"], stdinNull: true, timeout: 3)
        if res.exit != 0 {
            let err = res.err.trimmingCharacters(in: .whitespacesAndNewlines)
            NSLog("Sleepless: pmset displaysleepnow failed: %@", err.isEmpty ? "exit \(res.exit)" : err)
        }
    }
